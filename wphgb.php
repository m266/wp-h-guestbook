...
function wphgb_add_admin_menu() {
  add_menu_page('WP H-Guestbook', 'WP H-Guestbook', 'manage_options', 'wp-h-guestbook', 'wphgb_options_page', plugin_dir_url( __FILE__ ) . 'images/wphgb_icon.png');
...
function wphgb_settings_section_callback() {
  echo __('Bitte die Seiten-ID der Gästebuch-Seite eingeben (siehe Kurzanleitung unten):', 'wordpress');
}
function wphgb_options_page() {
  ?>
  <?php settings_errors(); ?> <!-- Meldung ausgeben -->
<!-- Eingabeformular fuer die Optionen -->
...
<h3>(Das Plugin ist auf <a href="https://www.web266.de/software/eigene-plugins/wp-h-guestbook/" target="_blank">web266.de</a> detailliert beschrieben)</h3>
  <?php
